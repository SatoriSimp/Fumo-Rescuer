    ...
    void Update()
    {
        ...
        if ((timeSinceLastMovement >= moveCooldown) && validTargets.Length <= 0 && movementDisabledCountdown <= 0 && !startAttacking)
        {
            ...
            if (!playerDetected)
                ...
                StartCoroutine(MoveToTarget(transform.position, playerDetected.position, Mathf.Max(Vector2.Distance(transform.position, playerDetected.position) / Mathf.Abs(moveDistance), 0.5f)));
        }
    }
